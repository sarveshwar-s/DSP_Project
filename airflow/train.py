...
def prepare_data():
    ...
    energy_data = pd.read_csv(my_dir + "/data/daily_dataset/block_0.csv")
    weather = pd.read_csv(my_dir + "/data/weather_dataset/weather_daily_darksky.csv")
...
def get_encoded_data(categorical_data: pd.DataFrame, isInference = False) -> np.ndarray:
    ...
    if not isInference:
        ...
        store_model(encoded_categorical_fit, my_dir + "/models/", isEncoder=True)
        encoded_categorical_data = encoded_categorical_data.toarray()
    else:
        cat_df = categorical_data
        cat_df = cat_df.dropna(axis=0)
        path = my_dir + "/models/encoder/encoder_1.joblib"
...
def store_model(model, model_path: str, isEncoder: bool = False) -> str:
    ...
    if not isEncoder:
        model_path = my_dir + "/"+ model_path + "model_1.joblib"
        dump(model, model_path)
    else:
        model_encoder_path = my_dir + "/models/encoder/encoder_1.joblib"
